...
<body>
	...
	<div class="container" id="container">
		...
		<div class="target-item-view">
			<div class="info">
				...
				<div class="name">${targetItem.name }</div>
				<div class="text">${targetItem.description }</div>
			</div>
			<div class="wish">
				<div class="icon"></div>
				<div class="text">想换: ${targetItem.wishItem}</div>
			</div>
			<div class="img">
		...
		<div class="divider-view">
			<c:if test="${isHasItems > 0 }">请选择交换物品</c:if>
			<c:if test="${isHasItems == 0 }">请先上传一个宝贝才能与Ta交换喔</c:if>
		</div>
		<div class="items">
		...
		<c:if test="${isHasItems == 0 }">
			<div class="btn-view">
				<button onclick="evens.onAddItemClick()">添加宝贝</button>
			</div>
		</c:if>
	</div>
    ...
    $(function () {
        ...
        evens.onAddItemClick = function(){
        	var eo = $("#eo").val();
        	var url = "addItem?eo="+eo;
        	location.href = url;
        }
